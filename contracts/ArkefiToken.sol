...
contract ArkefiToken is ERC20PresetMinterPauserUpgradeable, ERC20SnapshotUpgradeable, ERC20PermitUpgradeable {
    ...
    function _beforeTokenTransfer(
        ...
        require(to != address(this), "RKFI: Token transfer to this contract");
    ...
    function snapshot() public returns (uint256) {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "RKFI: Snapshot invalid role");
        require(!paused(), "RKFI: Contract paused");
        return _snapshot();
    }
    ...
    function batchMint(address[] calldata recipients, uint256[] calldata values) public returns (bool) {
        require(hasRole(MINTER_ROLE, _msgSender()), "RKFI: Batch mint invalid role");

        uint256 recipientsLength = recipients.length;
        require(recipientsLength == values.length, "RKFI: Batch mint not same legth");
        ...
        require(totalSupply() <= _cap, "RKFI: cap exceeded");
    ...
    function _mint(address account, uint256 amount) internal override {
        require(totalSupply() + amount <= _cap, "RKFI: cap exceeded");

...
describe("ArkefiToken", function () {
  ...
  it("can't transfer to contract address", async () => {
    ...
    await expect(token.transfer(token.address, tokens)).to.be.revertedWith("RKFI: Token transfer to this contract");
  });

  it("can't mint more than cap", async () => {
    const amountToMint = BigNumber.from(ethers.utils.parseEther('1000000001'));
    await expect(token.mint(admin.address, amountToMint)).to.be.revertedWith("RKFI: cap exceeded");
  });
  ...
  describe("Snapshot", () => {
    ...
    it("should fail to create snapshot when paused", async () => {
      await token.pause();
      await expect(token.snapshot()).to.be.revertedWith("RKFI: Contract paused");
    });
    ...
    it("shouldn't create snapshot if not admin", async () => {
      await expect(token.connect(deployer).snapshot()).to.be.revertedWith("RKFI: Snapshot invalid role");
    });
  });
  ...
  describe("BatchMint", function () {
    ...
    it("Can't batchMint without role", async () => {
      ...
      const role = await token.MINTER_ROLE();
      await token.revokeRole(role, admin.address);
      await expect(token.batchMint(recipients, tokens)).to.be.revertedWith("RKFI: Batch mint invalid role");
    });

    it("Can't batchMint with diferent length arrays", async () => {
      const tokens: BigNumber[] = [BigNumber.from(1)];
      const recipients: string[] = [deployer.address, admin.address];
      await expect(token.batchMint(recipients, tokens)).to.be.revertedWith("RKFI: Batch mint not same legth");
    });

    it("Can't batchMint to contract address", async () => {
      const tokens: BigNumber[] = [BigNumber.from(1)];
      const recipients: string[] = [token.address];
      await expect(token.batchMint(recipients, tokens)).to.be.revertedWith("RKFI: Token transfer to this contract");
    });
  });
});
